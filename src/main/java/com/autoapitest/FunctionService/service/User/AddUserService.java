...
@Service
public class AddUserService extends BasciService {

    String ADD_USER_URL="/userInfo/add";
     ...
     * @return
     */
    public JSONObject addUser(AddUserReq param, String result, Header[] headers, String envir) throws IOException {

        String BASEURL=new GetEnvironment().getUrl(envir,Item);
        //发送请求
        Response response = new GetResponse().postJson(BASEURL+ADD_USER_URL,param,null,headers);
        ...
        //失败则打印错误信息
        Assert.fail("addUser接口请求失败，错误码为："+response.getStatusCode());
        return null;
