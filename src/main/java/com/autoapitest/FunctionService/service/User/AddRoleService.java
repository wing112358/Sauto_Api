...
@Service
public class AddRoleService extends BasciService {

    String ADD_ROLE_URL="/roleInfo/add";
     ...
     * @return
     */
    public JSONObject addRole(AddRoleReq param, String result, Header[] headers, String envir) throws IOException {

        String BASEURL=new GetEnvironment().getUrl(envir,Item);
        //发送请求
        Response response = new GetResponse().postJson(BASEURL+ADD_ROLE_URL,param,null,headers);
        ...
        //失败则打印错误信息
        Assert.fail("addRole接口请求失败，错误码为："+response.getStatusCode());
        return null;
